...
namespace OCA\DrawioWhiteboard\Controller;
...
use OCA\DrawioWhiteboard\Migration;
...
class SettingsController extends Controller
{
     ...
     * @param OCA\DrawioWhiteboard\AppConfig $config - application configuration
     */
    ...
    public function settings()
    {
        ...
        if (version_compare(implode(".", \OCP\Util::getVersion()), "13", ">=")) {
            $checkmime = new \OCA\DrawioWhiteboard\Migration\CheckMimeType();
            $registered = $checkmime->run();

            if ($registered == false) {
            $mimeTypeLoader = \OC::$server->getMimeTypeLoader();
            $mime = new \OCA\DrawioWhiteboard\Migration\RegisterMimeType($mimeTypeLoader);
