 ...
 **/
    ...
    style("drawiowhiteboard", "editor");
    script("drawiowhiteboard", "editor");
    script("drawiowhiteboard", "viewer");
    ...
    <script type="text/javascript" nonce="<?php p(base64_encode($_["requesttoken"])) ?>" defer>
        window.addEventListener('DOMContentLoaded', function() {
            <?php if (!empty($_['error'])) { ?>
                OCA.DrawioWhiteboard.DisplayError("<?php p($_['error']) ?>");
            <?php } else { ?>
                ...
                OCA.DrawioWhiteboard.EditFile(iframe.contentWindow, filePath, originUrl, autosave);
